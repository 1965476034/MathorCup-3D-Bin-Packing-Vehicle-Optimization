...
# 九、问题1：单车型精确最少车辆 + 二次均衡优化（独立运行）
# 说明：
# - 与原先依赖外部基类的 q1 精确版不同，这里采用完全自包含实现；
# - G3 的支撑平台统一允许来自标准件 G1 / G2，符合附件约束；
# - 单车最优装载仍使用前面的多策略择优，最少车辆部分采用“模式库 + MILP 精确覆盖”。

PATTERN_POLICIES: Tuple[PackingPolicy, ...] = (
    ...
    PackingPolicy("mix_product", "product", {"standard": 1.3, "directional": 1.3, "fragile": 0.0}),
    PackingPolicy("directional_bias", "volume", {"directional": 2.5, "standard": 0.8, "fragile": 0.3}),
)

CAP_GRID: Tuple[Optional[int], ...] = (None, 30, 50, 100, 150, 200, 300)

_PATTERN_CACHE: Dict[Tuple[str, Tuple[int, ...], bool], List[SurfacePacker]] = {}


def _pattern_subsets() -> Tuple[Tuple[str, ...], ...]:
...
PATTERN_SUBSETS: Tuple[Tuple[str, ...], ...] = _pattern_subsets()


def _masked_counts(counts: Dict[str, int], subset: Sequence[str], cap: Optional[int]) -> Dict[str, int]:
    out: Dict[str, int] = {}
    for code in ITEM_ORDER:
...
def _pattern_key(packer: SurfacePacker) -> Tuple[int, ...]:
    return tuple(packer.loaded_counts[code] for code in ITEM_ORDER)


@lru_cache(maxsize=None)
def _pattern_min_fill_from_key(
    loaded_g1: int,
    loaded_g2: int,
    loaded_g3: int,
    loaded_g4: int,
...
def pattern_min_fill(packer: SurfacePacker) -> float:
    key = _pattern_key(packer)
    return _pattern_min_fill_from_key(*key, packer.truck.name)


def pattern_score_tuple(packer: SurfacePacker) -> Tuple[float, float, float, float, int, int]:
    util = packer.utilization()
    v = util["空间利用率_eff"]
    w = util["载重利用率"]
...
def pattern_secondary_score(packer: SurfacePacker) -> float:
    util = packer.utilization()
    v = util["空间利用率_eff"]
    w = util["载重利用率"]
    mix = sum(1 for code in ITEM_ORDER if packer.loaded_counts[code] > 0)
...
def sort_solution_by_fullness(packers: List[SurfacePacker]) -> List[SurfacePacker]:
    return sorted(packers, key=pattern_score_tuple, reverse=True)
...
def generate_pattern_library(
    ...
    best_by_pattern: Dict[Tuple[int, ...], SurfacePacker] = {}

    greedy_solution = greedy_pack_all(counts, catalog, truck, fragile_fixed=fragile_fixed)
    for packer in greedy_solution:
        best_by_pattern[_pattern_key(packer)] = packer

    for subset in PATTERN_SUBSETS:
        for cap in CAP_GRID:
            masked = _masked_counts(counts, subset, cap)
            ...
            for policy in PATTERN_POLICIES:
                ...
                if old is None or pattern_score_tuple(packer) > pattern_score_tuple(old):
                    best_by_pattern[key] = packer

    library = sorted(best_by_pattern.values(), key=pattern_score_tuple, reverse=True)
    _PATTERN_CACHE[cache_key] = library
    return library


def _build_count_matrix(packers: List[SurfacePacker]) -> np.ndarray:
    return np.array([[packer.loaded_counts[code] for packer in packers] for code in ITEM_ORDER], dtype=float)


def _target_vector(counts: Dict[str, int]) -> np.ndarray:
    return np.array([counts[code] for code in ITEM_ORDER], dtype=float)


def _run_milp_exact_cover(
    counts: Dict[str, int],
    ...
    matrix = _build_count_matrix(packers)
    target = _target_vector(counts)

    constraints: List[LinearConstraint] = [LinearConstraint(matrix, target, target)]
    if exact_truck_count is not None:
        ones = np.ones((1, len(packers)), dtype=float)
        exact = np.array([float(exact_truck_count)])
        constraints.append(LinearConstraint(ones, exact, exact))
...
def _extract_integer_coeffs(result) -> Optional[np.ndarray]:
...
def _coeffs_to_solution(packers: List[SurfacePacker], coeffs: np.ndarray) -> List[SurfacePacker]:
    ...
    return sort_solution_by_fullness(out)


def _solution_counts(packers: List[SurfacePacker]) -> Dict[str, int]:
...
def _solution_matches_target(packers: List[SurfacePacker], counts: Dict[str, int]) -> bool:
    return _solution_counts(packers) == {code: counts[code] for code in ITEM_ORDER}


def solve_min_truck_count_via_mip(
    counts: Dict[str, int],
    catalog: Dict[str, ItemType],
    truck: TruckType,
    fragile_fixed: bool = False,
) -> Tuple[int, List[SurfacePacker], List[SurfacePacker]]:
    library = generate_pattern_library(counts, catalog, truck, fragile_fixed=fragile_fixed)
    result = _run_milp_exact_cover(
        counts,
        library,
    ...
    coeffs = _extract_integer_coeffs(result)
    if coeffs is None:
        fallback = greedy_pack_all(counts, catalog, truck, fragile_fixed=fragile_fixed)
        return len(fallback), sort_solution_by_fullness(fallback), library

    solution = _coeffs_to_solution(library, coeffs)
    if not _solution_matches_target(solution, counts):
        fallback = greedy_pack_all(counts, catalog, truck, fragile_fixed=fragile_fixed)
        return len(fallback), sort_solution_by_fullness(fallback), library

    return len(solution), solution, library


def _highest_feasible_fill_threshold(
    counts: Dict[str, int],
    library: List[SurfacePacker],
    truck_count: int,
) -> float:
    values = sorted({pattern_min_fill(packer) for packer in library})
    if not values:
        return 0.0
    ...
    while lo <= hi:
        ...
        filtered = [packer for packer in library if pattern_min_fill(packer) + 1e-9 >= threshold]
        result = _run_milp_exact_cover(
            counts,
            filtered,
        ...
        if coeffs is not None:
            solution = _coeffs_to_solution(filtered, coeffs)
            if _solution_matches_target(solution, counts):
                best = threshold
                lo = mid + 1
                continue
        hi = mid - 1
...
def optimize_balance_at_fixed_truck_count(
    counts: Dict[str, int],
    library: List[SurfacePacker],
    truck_count: int,
) -> List[SurfacePacker]:
    threshold = _highest_feasible_fill_threshold(counts, library, truck_count)
    filtered = [packer for packer in library if pattern_min_fill(packer) + 1e-9 >= threshold]
    if not filtered:
        filtered = library

    objective = -np.array([pattern_secondary_score(packer) for packer in filtered], dtype=float)
    result = _run_milp_exact_cover(
        counts,
        filtered,
        objective=objective,
    ...
    fallback_result = _run_milp_exact_cover(
        counts,
        library,
        objective=-np.array([pattern_secondary_score(packer) for packer in library], dtype=float),
        exact_truck_count=truck_count,
        time_limit=60.0,
    )
    fallback_coeffs = _extract_integer_coeffs(fallback_result)
    if fallback_coeffs is not None:
        fallback_solution = _coeffs_to_solution(library, fallback_coeffs)
        if _solution_matches_target(fallback_solution, counts):
...
def solve_min_trucks_single_type(
    ...
    baseline = sort_solution_by_fullness(greedy_pack_all(counts, catalog, truck, fragile_fixed=fragile_fixed))
    best_count, stage1_solution, library = solve_min_truck_count_via_mip(counts, catalog, truck, fragile_fixed=fragile_fixed)
    balanced = optimize_balance_at_fixed_truck_count(counts, library, best_count)

    if balanced:
        return sort_solution_by_fullness(balanced)
    if stage1_solution:
        return sort_solution_by_fullness(stage1_solution)
    return baseline


def solve_question1(fragile_fixed: bool = False) -> Dict[str, object]:
    trucks, catalog = build_problem_data()
    counts = counts_from_catalog(catalog)

    truck1, truck2 = trucks

    best_truck1 = pack_single_truck_bestof(counts, catalog, truck1, fragile_fixed=fragile_fixed)
    best_truck2 = pack_single_truck_bestof(counts, catalog, truck2, fragile_fixed=fragile_fixed)

    min_solution_truck1 = solve_min_trucks_single_type(counts, catalog, truck1, fragile_fixed=fragile_fixed)
    min_solution_truck2 = solve_min_trucks_single_type(counts, catalog, truck2, fragile_fixed=fragile_fixed)

    return {
        "catalog": catalog,
        "single_truck_best": {
            truck1.name: [best_truck1],
            truck2.name: [best_truck2],
        },
        "min_trucks_single_type": {
            truck1.name: min_solution_truck1,
            truck2.name: min_solution_truck2,
        },
    }


if __name__ == "__main__":
    result = solve_question1(fragile_fixed=False)
    print_solution_summary("Q1(1) 车型1单车最优装载", result["single_truck_best"]["车型1"])
    print_solution_summary("Q1(1) 车型2单车最优装载", result["single_truck_best"]["车型2"])
    print_solution_summary("Q1(2) 仅用车型1时车辆数最少方案", result["min_trucks_single_type"]["车型1"])
    print_solution_summary("Q1(2) 仅用车型2时车辆数最少方案", result["min_trucks_single_type"]["车型2"])
